...
class TestPhase1Endpoints:
    ...
    def test_database_connection_test_error_handling(self):
        """Test database connection test endpoint error handling"""
        # Test without proper database connection - should return error
        response = client.get("/api/v1/phase1/connect")
        print(f"Response status: {response.status_code}")
        print(f"Response content: {response.text}")
        
        # During migration, expect 503 with database error
        assert response.status_code == 503
        assert "Database" in response.text or "database" in response.text
...
class TestPhase2Endpoints:
    ...
    def test_normalization_status(self):
        ...
        response = client.post("/api/v1/phase2/normalization-status")
        assert response.status_code == 200
