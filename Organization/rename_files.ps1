...
# References folder - keeping original CLI patterns but ensuring consistency
$referencesPath = "D:\DigiFloat\LawChronicle\references"
if (Test-Path $referencesPath) {
    Write-Host "📁 Processing References Folder (maintaining CLI patterns)..." -ForegroundColor Cyan
    
    # GUI comparison files - these follow good CLI patterns, just ensure consistency
    $guiPath = "$referencesPath\gui"
    Get-ChildItem "$guiPath\comparison-*.json" | ForEach-Object {
        # These already follow good patterns, no change needed
        Write-Host "✅ CLI pattern OK: $($_.Name)" -ForegroundColor Gray
    }
    
    # Filled files - also good patterns
    Get-ChildItem "$guiPath\filled-*.txt" | ForEach-Object {
        Write-Host "✅ CLI pattern OK: $($_.Name)" -ForegroundColor Gray
    }
}
